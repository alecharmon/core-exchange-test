...
package openapi
// TransactionStatus : The status of a transaction. Plaid consumes solely the `PENDING` and `POSTED` enums, and treats `MEMO` and `AUTHORIZATION` as if they were `PENDING`. Plaid expects that pending and posted transactions have different `transactionIds`. * `AUTHORIZATION` * `MEMO` - A pending transaction to be completed at the end of this day * `PENDING` - A pending transaction * `POSTED` - A posted transaction 
type TransactionStatus string

// List of TransactionStatus
const (
	AUTHORIZATION TransactionStatus = "AUTHORIZATION"
